...
class case(wx.Window):
    ...
    def __init__(self, parent):
        ...
        self.font_jeton = wx.Font(
            self.settings["size_font_jeton"],
            wx.FONTFAMILY_DEFAULT,
            wx.FONTSTYLE_NORMAL,
            wx.FONTWEIGHT_BOLD,
        )
        self.font_point = wx.Font(
            self.settings["size_font_point"],
            wx.FONTFAMILY_DEFAULT,
            wx.FONTSTYLE_NORMAL,
            wx.FONTWEIGHT_BOLD,
        )
    ...
    def OnPaint(self, evt):
        ...
        if not self.is_vide():
            ...
            if self.get_jeton_status() in (jeton.TEMP, jeton.PREPOSE):
                ...
                font = (
                    "fontcol_tempjoker"
                    if self._jeton.is_joker()
                    else "fontcol_tempnorm"
                )
            else:
                col = s["col_pose"]
                font = (
        ...
        # ou la flèche
            ...
            if self._fleche == coord.HOR:
                pts_reel = [
                    (int(x * self.size / 10), int(y * self.size / 10)) for (x, y) in pts
                ]
            elif self._fleche == coord.VER:
                pts_reel = [
                    (int(y * self.size / 10), int(x * self.size / 10)) for (x, y) in pts
                ]
